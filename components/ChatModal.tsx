...
import { startChatSession, sendMessage, ChatSession } from '../services/geminiService';
...
const ChatModal: React.FC<ChatModalProps> = ({ product, onClose }) => {
  ...
  useEffect(() => {
    const initChat = async () => {
        setIsLoading(true);
        const session = startChatSession(product);
        setChatSession(session);
        
        // Send an initial message to the model to get a greeting
        const initialModelMessage = await sendMessage(session, "Hello, please greet me and ask how you can help with this product.");
        
        setMessages([
            { role: 'model', text: initialModelMessage || `Hello! How can I help you with the ${product.name} today?` }
        ]);
        setIsLoading(false);
    };
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
            <div className="flex items-center">
                {product.imageUrl ? (
                    <img src={product.imageUrl} alt={product.name} className="w-10 h-10 object-cover rounded-full mr-3" />
                ) : (
                    <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center mr-3">
                        <Icon name="store" className="h-5 w-5 text-gray-400" />
                    </div>
                )}
                <div>
                    <h2 className="font-bold text-brand-dark">{product.name}</h2>
